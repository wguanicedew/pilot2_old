...
class EventServiceManager(threading.Thread):
    ...
    def __init__(self):
        self.__message_queue_to_payload = Queue.Queue()
        self.__message_queue_from_payload = Queue.Queue()
    ...
    def init(self, socketname='EventService_EventRanges', context='local', athenaMPCmd=None):
        ...
        if child_pid == 0:
            # child process
            self.initMessageThread(socketname=socketname, context=context)
